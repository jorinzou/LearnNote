...
void* input_poll(void *arg)
{
    ...
    while(1) {
        ...
        if (poll_fd[0].revents == POLLIN){
            ...
            DEBUG_INFO("client send ret=%d,send buf:%s",ret,buf);
         }
		if (poll_fd[1].revents == POLLIN){
			int data = 0;
			ret = recvfrom(sockefd,&data,sizeof(data),MSG_DONTWAIT/*非阻塞*/,NULL,0); 
			DEBUG_INFO("client recv ret=%d,data=%d",ret,data);
		}
    }
}
