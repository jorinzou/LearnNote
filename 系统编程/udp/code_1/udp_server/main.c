...
{
    unsigned char buf[] = {"udp server send data"};
    while(1){
        int cmd = 0;
        scanf("%d",&cmd);
        if (cmd == 1){
           connect_init("/tmp/udp_local_client");
           int ret = write(udp_sockfd,buf,sizeof(buf)); 
           if(ret < 0){
               perror("write failed");
...
int main(void)
{
    udp_sockfd = udp_sock_init("/tmp/udp_local_server");
    if (udp_sockfd < 0){
